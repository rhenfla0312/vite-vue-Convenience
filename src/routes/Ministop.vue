    ...
    search() {
      ...
      if(this.oneData) {
        ...
        axios({
          ...
          console.log(res)
          this.datas = res.data.results
          localStorage.getItem('search__data')
          if(res.data.next) {
            this.nextData = res.data.next
            this.Loading = true
            localStorage.getItem('search__data')
  ...
  mounted() {
    if(localStorage.getItem('search__data') !== "") {
      ...
      if(localStorage.getItem('oneClick')) {
        ...
        axios({
          method: "GET",
          url: `http://54.180.193.83:8081/Main/event/MINISTOP/?search=${localStorage.getItem('search__data')}`
        }).then((res) => {
          console.log(res)
          this.datas = res.data.results
          if(res.data.next) {
  ...
  <div class="convenience">
    <div class="inner">
      <div class="convenience__name">
        ...
        <RouterLink @click="search__data()" to="/convenience/gs" class="__name">GS25</RouterLink>
        <RouterLink @click="search__data()" to="/convenience/ministop" class="__name __ministopName">MINISTOP</RouterLink>
      </div>
      <!-- 검색 -->
